...
`readAOFM` helps R users download and tidy workbook data published by
the [Australian Office of Financial Management
(AOFM)](https://www.aofm.gov.au/) Data Hub. It is for analysts,
researchers, and reproducible reports that need Australian Government
securities, issuance, ownership, turnover, or term-premium data in
R-friendly tabular shapes.

This is the development version from GitHub. It is not yet a CRAN
release; the CRAN installation command below applies only after CRAN
acceptance.
...
After acceptance on CRAN, use:

``` r
install.packages("readAOFM")
```

## First success without a network request

`search_aofm()` searches the local table catalogue. Its default
`read = FALSE` does not contact AOFM and returns ready-to-run calls.

``` r
if (requireNamespace("readAOFM", quietly = TRUE)) {
...
## First parsed table, still offline

The package ships a small official workbook snapshot for deterministic
documentation and tests. This example replaces only the internal
downloader; it still runs the public dispatcher and transactional
parser, without making a network request.
...
if (requireNamespace("testthat", quietly = TRUE) && nzchar(fixture)) {
  ...
  data.frame(
    ...
    first_tender = min(tb_issuance$date_held),
    latest_tender_in_snapshot = max(tb_issuance$date_held)
  )
}
#>    rows columns first_tender latest_tender_in_snapshot
#> 1 26268       7   1982-08-05                2025-05-27
```

Packaged snapshots are examples, not a runtime cache or a substitute for
the current AOFM source. Normal reads fetch the live workbook as shown
next.

## Read a live table

Readers fetch the current workbook from AOFM. This live example is
guarded so that it runs when copied into an interactive R session but
never makes a network request while the README is built or during CRAN
checks.

``` r
if (interactive()) {
  x <- readAOFM::read_aofm("tb", "issuance")
  stopifnot(is.data.frame(x), all(c("name", "value") %in% names(x)))
  utils::head(x, 3)
}
```

`read_aofm()` returns one parsed object for one match and a named list
when selectors match several tables. The default call selects all parsed
tables and can download a substantial amount of data, so start with a
specific pair.

## Supported AOFM data

The current catalogue contains 23 parsed tables. These table IDs are
stable catalogue identifiers; the public `read_aofm()` interface
normally uses their `security` and `type` selectors.

| AOFM family | Parsed table IDs | Reader and shape |
|----|----|----|
| End-of-financial-year summary | `summary` | `read_eofy()`; one long data frame |
| End-of-month positions | `aggregate_position_dealt`, `aggregate_position_settlement`, `tb_position_dealt`, `tb_position_settlement`, `tib_position_dealt`, `tib_position_settlement`, `tn_position_dealt`, `tn_position_settlement` | `read_eom()`; named list of position data frames |
| Transactional issuance/buybacks | `tb_issuance`, `tb_buyback`, `tib_issuance`, `tib_buyback`, `tn_issuance`, `retail`, `slf` | `read_transactional()`; one long data frame |
| Syndication details | `tb_syndication`, `tib_syndication` | `read_syndication()`; one long data frame |
| Ownership | `ownership_public`, `ownership_nonresident` | `read_ownership()`; named list of data frames |
| Secondary-market turnover | `tb_turnover`, `tib_turnover` | `read_secondary()`; one long data frame |
| Term premium | `termpremium` | `read_premium()`; one long data frame |

Seven catalogue entries have no parser in this version and are
raw-download-only metadata: `tb_issuance_conversion`,
`indexation_factors`, `rmbs_transactions`, `rmbs_auctions`,
`interest_rate_swaps`, `cross_currency_swaps`, and `portfolio_overview`.
They are excluded from the supported catalogue searched by
`search_aofm()`; because that function falls back to token matching, a
raw-only ID can still coincidentally match a supported row. Only IDs in
the returned rows are readable. The raw-only entries’ selector fields
are empty, so they cannot be requested individually; the current raw
helper includes them only when `download_aofm_xlsx()` is called without
either selector, which also downloads every other catalogue workbook.
...
`download_aofm_xlsx()` helper saves raw workbooks under `data/` and
prints selected table IDs.
...
## Network, credentials, and cache behavior
...
- Downloads use a 15-second connection timeout, a 120-second transfer
  limit, a low-speed abort after 30 seconds below 1 KiB/s, and a 100 MiB
  file limit.
- No AOFM username, password, API key, or other credential is required
  or collected.
- Readers use a temporary workbook for parsing and do not provide a
  persistent download cache. Repeating a read can fetch a newer
  workbook.
- `csv = TRUE` writes to `output/` and `download_aofm_xlsx()` writes to
...
The package does not parse the seven raw-only IDs above, does not accept
a local workbook path through public readers, and does not promise
identical identifier columns across source families. It is not a
...
Use `citation("readAOFM")` for the package citation.
