...
searches the local table catalogue. Its default `read = FALSE` does not
contact AOFM and returns ready-to-run calls.
...
returns one parsed object for one match and a named list when selectors
match several tables. The default call selects all parsed tables and can
download a substantial amount of data, so start with a specific pair.

## Supported AOFM data

The current catalogue contains 23 parsed tables. These table IDs are
stable catalogue identifiers; the public
[`read_aofm()`](https://joel23978.github.io/readAOFM/reference/read_aofm.md)
interface normally uses their `security` and `type` selectors.
...
[`search_aofm()`](https://joel23978.github.io/readAOFM/reference/search_aofm.md);
because that function falls back to token matching, a raw-only ID can
still coincidentally match a supported row. Only IDs in the returned
rows are readable. The raw-only entries’ selector fields are empty, so
they cannot be requested individually; the current raw helper includes
them only when
[`download_aofm_xlsx()`](https://joel23978.github.io/readAOFM/reference/download_aofm_xlsx.md)
is called without either selector, which also downloads every other
catalogue workbook.
